...
test("renders the subscription club page", async () => {
  ...
  assert.match(html, /Economize[\s\S]{0,40}R\$\s*10/);
  assert.match(html, /Economize[\s\S]{0,40}R\$\s*46/);
...
test("renders the Linha Rotina page and its premium value ladder", async () => {
  ...
  assert.match(html, /1 a 4 refeições/);
  assert.match(html, /5 a 19 refeições/);
  assert.match(html, /20 a 30 refeições/);
  assert.match(html, /R\$\s*27,80/);
  assert.match(html, /R\$\s*26,45/);
